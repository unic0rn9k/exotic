...
macro_rules! activation {
    ([$($T: ty),*]: $name: ident = $f: expr, $d: expr $(,)?) => {
        ...
        impl<const LEN: usize> Layer<$T, LEN, LEN> for $name<$T, LEN> {
            fn predict(&mut self, i: impl StaticVec<$T, LEN>) -> Result<[$T; LEN]> {
                let fun: fn($T)->$T = $f;
...
impl<T: Float + std::iter::Sum, const LEN: usize> Layer<T, LEN, LEN> for Softmax<T, LEN> {
    fn predict(&mut self, i: impl StaticVec<T, LEN>) -> Result<[T; LEN]> {
        let mut buffer = [num!(0); LEN];
